...
double last_vio_t = -1;
std::queue<geometry_msgs::PoseStampedConstPtr> gpsQueue;
std::mutex m_buf;
...
void GPS_callback(const geometry_msgs::PoseStampedConstPtr &GPS_msg)
{
    //printf("gps_callback! \n");
    m_buf.lock();
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    {
        geometry_msgs::PoseStampedConstPtr GPS_msg = gpsQueue.front();
        ...
        if(gps_t >= t - 0.01 && gps_t <= t + 0.01)
        {
            ...
            double altitude = GPS_msg->pose.position.z;

            //int numSats = GPS_msg->status.service;
            // double pos_accuracy = GPS_msg->position_covariance[0];
            // if(pos_accuracy <= 0)
            double pos_accuracy = 0.1;
    ...
    //change
    // Eigen::Quaterniond rotation_q(Eigen::AngleAxisd(M_PI, Eigen::Vector3d(1, 0, 0)));
    // Eigen::Quaterniond result_q = rotation_q * global_q;
    //change
    ...
    odometry.twist.twist.linear.x = pose_msg->twist.twist.linear.x;
    odometry.twist.twist.linear.y = pose_msg->twist.twist.linear.y;
    odometry.twist.twist.linear.z = pose_msg->twist.twist.linear.z;
...
int main(int argc, char **argv)
{
    ...
    // ros::Subscriber sub_GPS = n.subscribe("/airsim_node/drone_1/debug/pose_gt", 100, GPS_callback);
    ros::Subscriber sub_GPS = n.subscribe("/gps_vins", 100, GPS_callback);
