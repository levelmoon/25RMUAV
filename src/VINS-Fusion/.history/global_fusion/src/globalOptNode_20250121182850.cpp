...
double last_vio_t = -1;
std::queue<geometry_msgs::Pose::ConstPtr> gpsQueue;
std::mutex m_buf;
 ...
 * 订阅GPS
*/
void GPS_callback(const geometry_msgs::Pose::ConstPtr &GPS_msg)
{
    //printf("gps_callback! \n");
    m_buf.lock();
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    {
        // geometry_msgs::NavSatFixConstPtr GPS_msg = gpsQueue.front();
        geometry_msgs::Pose::ConstPtr GPS_msg = gpsQueue.front();
        ...
        // {
            ...
            // 经纬度、海拔
            double latitude = GPS_msg->position.x;
            double longitude = GPS_msg->position.y;
            double altitude = -GPS_msg->position.z;
